import {parse_utf16, font_data} from "./main.js";
...
// TODO: automatically detect when to draw the preview and the main window?



export let preview_status = {
    cx: 12,
    cy: 12,
...
function preview_draw_glyph(glyph, codepoint, x, y) {
    ...
    let sx = Math.round(preview_status.cx + x * (font_data.width + font_data.spacing) * pixel_size);
    let sy = Math.round(preview_status.cy + y * font_data.height * pixel_size);
...
export function draw() {
    ...
    for (let codepoint of parse_utf16(preview_status.text)) {
        if (codepoint === 10) { // "\n"
            y++;
            x = 0;
            continue;
        }
        let glyph = font_data.glyphs.get(codepoint);
        if (glyph) {
